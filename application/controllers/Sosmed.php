<?php
    ...
    class Sosmed extends CI_Controller
    {
        ...
        function index(){
            $konten['css']		    = '';
			$konten['konten'] 		= 'p_sosmed/view_sosmed';
			$konten['judul']		= 'Dashboard';
			$konten['sub_judul'] 	= 'Data Sosial Media';
			$konten['data']			= $this->model_dinamic->getData('tb_sosmed');
			$konten['js']			= '
		...
		function tambah (){
			$konten['css']			= '
				...
				';
			$konten['konten'] 		= 'p_sosmed/form_tambah_sosmed';
			$konten['judul']		= 'Dashboard';
			$konten['sub_judul'] 	= 'Tambah Data sosmed';
		...
		public function simpan() // simpan data berita
		{
			...
			$data['id'] 				= $key;
			$data['jenis_sosmed'] 		= $this->input->post('jenis');
			$data['nama_sosmed'] 		= $this->input->post('nama');
			$data['link_website']		= $this->input->post('link');
			
			$query = $this->model_sosmed->getdata($key);
				if($query->num_rows()>0)
				{
					
					$this->model_sosmed->getupdate($key,$data);
				...
				{
					// print_r($data);
					$this->model_sosmed->getinsert($data);
					$this->session->set_flashdata('info','Data berhasil di simpan');
				}
				redirect('admin/pengaturan/sosmed');
			}
		...
		{
			$key = $id;
			$query = $this->model_dinamic->getWhere ('tb_sosmed','id',$id);
			if($query->num_rows()>0)
				{
					
					$this->model_sosmed->getdelete($key);
		...
		public function ubah($id) // Mengubah data berita
		{
			...
			$konten['js']			= '
				...
				';
			$konten['konten'] 		= 'p_sosmed/form_ubah_sosmed';
			$konten['judul']		= 'Dashboard';
			$konten['sub_judul'] 	= 'Ubah Data sosmed';
			$key = $id;
			$query = $this->model_dinamic->getWhere('tb_sosmed','id',$key);
			if($query->num_rows()>0)
			
			{
				...
				foreach ($query->result() as $row )
					{
						$konten['kode']			= $row->id;
						$konten['nama']			= $row->nama_sosmed;
						$konten['jenis']		= $row->jenis_sosmed;
