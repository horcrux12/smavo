<?php
    ...
    class Slider extends CI_Controller
    {
        ...
        function index(){
			...
			$konten['konten'] 		= 'p_slider/view_slider';
			$konten['judul']		= 'Dashboard';
			$konten['sub_judul'] 	= 'Data slider';
			$konten['data']			= $this->model_dinamic->getData('tb_slider');
			...
			// $konten['data']	= $this->model_slider->tampil_slider()->result();
			// print_r($konten['data']); 
		...
		function tambah (){
			$konten['css']			= '
				...
				';
			$konten['konten'] 		= 'p_slider/form_tambah_slider';
			$konten['judul']		= 'Dashboard';
			$konten['sub_judul'] 	= 'Tambah Data slider';
		...
		public function simpan() // simpan data slider
		{
		...
		//   else {
			...
			$data['id'] 				= $key;
			$data['judul_utama'] 		= $this->input->post('jdl');
			$data['sub_judul'] 			= $this->input->post('sub');
			$data['deskripsi']			= $this->input->post('isi');
			$data['foto']				= $this->upload->data('file_name');
			
			$query = $this->model_slider->getdata($key);
				if($query->num_rows()>0)
				{
					
					$this->model_slider->getupdate($key,$data);
			...
			redirect('admin/pengaturan/slider');
			}
		...
		{
			$key = $id;
			$query = $this->model_dinamic->getWhere ('tb_slider','id',$id);
			if($query->num_rows()>0)
				{
					$query=$query->result();
					unlink('./assets/photo/slider/'.$query[0]->foto);
					$this->model_slider->getdelete($key);
					$this->session->set_flashdata('info_hapus','Data berhasil di hapus');
				}
				redirect('admin/pengaturan/slider');
		}

		public function ubah($id) // Mengubah data slider
		{
			$konten['css']			= '
				<link rel="stylesheet" href="'.base_url().'assets/css/jquery-ui.custom.min.css" />
				<link rel="stylesheet" href="'.base_url().'assets/css/chosen.min.css" />
			...
			$konten['js']			= '
				...
				';
			$konten['konten'] 		= 'p_slider/form_ubah_slider';
			$konten['judul']		= 'Dashboard';
			$konten['sub_judul'] 	= 'Ubah Data slider';
			$key = $id;
			$query = $this->model_dinamic->getWhere('tb_slider','id',$key);
			if($query->num_rows()>0)
			
			{
				...
				foreach ($query->result() as $row )
					{
						$konten['kode']			= $row->id;
						$konten['jdl']			= $row->judul_utama;
						$konten['sub']			= $row->sub_judul;
